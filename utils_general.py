...
def get_acc_loss(data_x, data_y, model, dataset_name, w_decay = None):
    ...
    if (dataset_name == "CIFAR10"):
        data_x = data_x.reshape((-1, 3, 32, 32))
...
def train_scaffold_mdl(model, model_func, state_params_diff, trn_x, trn_y, 
    ...
    if dataset_name == "FMNIST_off":
        trn_gen = data.DataLoader(Dataset(trn_x, trn_y, train=True, dataset_name=dataset_name), batch_size=batch_size, shuffle=True, collate_fn=custom_collate)
    else:
        trn_gen = data.DataLoader(Dataset(trn_x, trn_y, train=True, dataset_name=dataset_name), batch_size=batch_size, shuffle=True)
    loss_fn = torch.nn.CrossEntropyLoss(reduction='sum')
...
def set_client_from_params(mdl, params):
    torch.save(mdl.state_dict(), tmp_save_model_path)
    dict_param = torch.load(tmp_save_model_path)
    idx = 0
