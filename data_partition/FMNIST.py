...
# dataset下载目录
DATASET_FOLDER = config.ROOT_PATH + "data/FMNIST/"
...
class FMnistShardDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # x = np.array(self.dataset.iloc[:, 1:]).reshape(-1, 1, 28, 28).astype(float)[idx]
        # y = np.array(self.dataset.iloc[:, 0])[idx]
...
# 0
def get_FMNIST_dataloaders(dataset_name, batch_size: int, shuffle=True, reset_data=False):
    """得到训练集DataLoader对象列表
    ...
    elif dataset_name == "FMNIST_shard":
        n_clients = 100
    ...
    # dirichlet
    elif dataset_name[:6] == "FMNIST":
        n_classes = 10
        n_clients = 100
        balanced = dataset_name[7:11] == "bbal"
        alpha = float(dataset_name[12:])
