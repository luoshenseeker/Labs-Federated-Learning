...
from py_func.FedProx import loss_classifier
from py_func.FedProx import accuracy_dataset
from py_func.FedProx import loss_dataset
from py_func.FedProx import save_pkl

from config import tmp_save_model_path

### Methods
def train_SCAFFOLD(data_obj, act_prob ,learning_rate, batch_size, n_minibatch, file_name,
                                     com_amount, print_per, weight_decay, n_sampled,
                                     model_func, init_model, sch_step, sch_gamma, 
                                     save_period, metric_period=1, suffix = '', trial=True, data_path='', rand_seed=0, lr_decay_per_round=1, global_learning_rate=1):
    ...
    n_samples = np.array([len(db.dataset) for db in data_obj.train_set])
    weights = n_samples / np.sum(n_samples)
    print("Clients' weights:", weights)

    loss_hist = np.zeros((com_amount + 1, n_clnt))
    acc_hist = np.zeros((com_amount + 1, n_clnt))

    for k, dl in enumerate(data_obj.train_set):
        loss_hist[0, k] = float(loss_dataset(init_model, dl, loss_classifier).detach())
        acc_hist[0, k] = accuracy_dataset(init_model, dl)
    ...
    if (trial) or (not os.path.exists('%sModel/%s/%s/%dcom_sel.pt' %(data_path, data_obj.name, suffix, com_amount))):         
        ...
        if saved_itr == -1:
            if file_name[:6] == "FMNIST":
                torch.save(init_model.state_dict(), tmp_save_model_path)
                avg_model = model_func().to(device)
                avg_model.load_state_dict(torch.load(tmp_save_model_path))

                all_model = model_func().to(device)
                all_model.load_state_dict(torch.load(tmp_save_model_path))
            else:
                avg_model = model_func().to(device)
        ...
        for i in range(saved_itr+1, com_amount):
            ...
            #         break
            np.random.seed(i)
            selected_clnts = random.sample([x for x in range(n_clnt)], n_sampled)
            ...
            for clnt in selected_clnts:
                ...
                torch.save(avg_model.state_dict(), tmp_save_model_path)
                clnt_models[clnt].load_state_dict(torch.load(tmp_save_model_path))
            ...
            if i % metric_period == 0:
                ...
                for k, dl in enumerate(data_obj.train_set):
                    loss_hist[i + 1, k] = float(
                        loss_dataset(all_model, dl, loss_classifier).detach()
                    )

                for k, dl in enumerate(data_obj.test_set):
                    acc_hist[i + 1, k] = accuracy_dataset(all_model, dl)

                server_loss = np.dot(weights, loss_hist[i + 1])
                server_acc = np.dot(weights, acc_hist[i + 1])

                print(
                    f"====> i scaffold: {i+1} Loss: {server_loss} Server Test Accuracy: {server_acc}"
                )
        ...
        torch.save(
            avg_model.state_dict(), f"saved_exp_info/final_model/{file_name}.pth"
        )
