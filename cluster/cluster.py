...
# root_path = f"{config.ROOT_PATH}saved_exp_info/data_partition_result/MNIST_nbal_0.01.pkl"
...
#             break
# print(len(m_data))
# 第一个参数为包含k个元素的元组形式,表示在每个维度上a的值,第二个表示sample个数
s = np.random.dirichlet(range(1,11), 50000)
data = s
# # 1.2 对数据进行零均值标准化处理
# for d in m_data:
#     da = []
...
for i in range(5, 6):  # 设置聚类簇数为10个
    ...
    # print(result)
    for j in pred_y:
        pred_y_colors.append(colors[j])
    # save_path = f'{config.ROOT_PATH}saved_exp_info/cluster_result/cifar10_balance_dir.pkl'
...
#     # plt.show()
    # 绘制聚类效果图(2D版本)
    plt.scatter(data[:,0],data[:,1],c=pred_y_colors,marker='o',s=0.3,linewidths=0)
    # plt.title("k-Means")
    plt.axis('off')
    plt.savefig('cluster_50000.pdf', format='pdf', dpi=500, bbox_inches="tight")
    plt.show()
